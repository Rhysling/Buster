...
let formatNum = function (num, len) {
  ...
  if (sl < len)
    return "0".repeat(len - sl) + str;

  return str;
};

var d = new Date();
var key = formatNum(d.getFullYear(), 4) + formatNum(d.getMonth() + 1, 2) + formatNum(d.getDate(), 2) + "-" + formatNum(d.getHours(), 2)+ formatNum(d.getMinutes(), 2)+ formatNum(d.getSeconds(), 2);
console.log("Key: ", key);
...
fs.readFile(".\\public\\index.html", "utf8")
  .then(a => {
    return a.replace(/bundle\.css/g, bundleName + ".css").replace(/bundle\.js/g, bundleName + ".js");
  })
  .then(a => {
